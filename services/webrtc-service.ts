...
import { API_BASE_URL, ICE_SERVERS, RECORDING_MIME_TYPES, RECORDING_OPTIONS } from "@/utils/constants"
...
export class WebRTCService {
  ...
  // 6. Callback handlers
  private onCallStatusChange: ((status: string, data?: any) => void) | null = null
  private onIncomingCall: ((data: any) => void) | null = null
  private onCallEnded: (() => void) | null = null
  private onRecordingStatusChange: ((isRecording: boolean) => void) | null = null
  ...
  async initiateCall(receiverId: string, appointmentId: string, isVideo = false) {
    try {
  ...
  checkRecordingSupport() {
    const supportedType = RECORDING_MIME_TYPES.find((type) => MediaRecorder.isTypeSupported(type))
  ...
  async startRecording() {
    ...
    try {
      ...
      if (this.remoteStream) {
        const remoteAudioSource = this.audioContext.createMediaStreamSource(this.remoteStream)
      ...
      const drawVideoFrame = () => {
        ...
        if (this.remoteVideoElement && this.remoteVideoElement.readyState >= 2) {
          this.canvasContext.drawImage(this.remoteVideoElement, 0, 0, this.canvas!.width / 2, this.canvas!.height)
        }
      ...
      this.mediaRecorder = new MediaRecorder(combinedStream, {
        ...
        videoBitsPerSecond: this.isVideoCall ? RECORDING_OPTIONS.videoBitsPerSecond : undefined,
      })
      ...
      this.mediaRecorder.onstop = () => {
        ...
        const blob = new Blob(this.recordedChunks, { type: supportedMimeType as string })
        ...
        if (!this.recordingFinalized && this.recordingId && this.currentAppointment) {
